import { categoryNamesEquivalent, detectCategory, findBrandHit, findCategoryHit } from './categoryMatch';
...
export function matchProducts(query, products, priceRange, limit = 10, dbCategories = [], contextCategory = null, hiddenCategories = []) {
  ...
  const hiddenSet = new Set((hiddenCategories || []).map((c) => (c || '').trim().toLowerCase()));
  ...
  if (categoryHit) {
    // Hidden category — checked BEFORE any product filtering below, so a
    // hidden category always gets the graceful "updating" message and never
    // an empty/confusing result from the zeroInCategoryHit path, regardless
    // of whether any of its products are individually still active.
    if (hiddenSet.has(categoryHit.trim().toLowerCase())) {
      console.log(`🙈 matchProducts: category hit "${categoryHit}" is hidden — graceful message, no product search`);
      return { matched: [], upsells: [], offTopic: false, zeroInCategoryHit: true, categoryHit, usedFamilyFallback: false, family: null };
    }
    const categoryFiltered = products.filter((p) => categoryNamesEquivalent(p.category, categoryHit) && p.is_active !== false);
  ...
  let undetectedCategoryFamily = null;
  if (detectedCat !== 'Other') {
    categoryFiltered = products.filter((p) => categoryNamesEquivalent(p.category, detectedCat));
    ...
    if (familyFiltered.length > 0) {
      ...
      undetectedCategoryFamily = family;
  ...
  return {
    ...
    family: undetectedCategoryFamily,
  };
}
