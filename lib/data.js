...
// Customer-facing — hidden categories never appear here.
export async function getCategories() {
  const all = await getAllCategoriesRaw();
  return all.filter((c) => c.is_active !== false);
}

// Unfiltered — used only where a page needs to tell "category is hidden"
// apart from "category doesn't exist" (e.g. to 404 a hidden category's
// direct URL instead of silently rendering the empty-category fallback).
export async function getAllCategoriesIncludingHidden() {
