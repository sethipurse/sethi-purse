...
export async function getProducts() {
  try {
    ...
    // Only fall back to local if Supabase itself errors (not just empty)
    if (error) return LOCAL_PRODUCTS.filter((p) => p.is_active !== false);
    // If Supabase returned rows, use them. If empty, use local.
...
export async function getCategories() {
  try {
    ...
    if (error) return LOCAL_CATEGORIES;
...
export async function getOffers() {
  try {
    const { data, error } = await supabase
      .from('offers')
      .select('*')
      .eq('is_active', true)          // ← filter in Supabase directly
      .order('created_at', { ascending: false });
...
export async function getReviews() {
  try {
    ...
    // reviews table has no is_approved column — just return all
    return source;
  } catch {
