...
// Display-only formatting for category names stored in ALL CAPS (e.g.
// "LUGGAGE") so they render consistently with normally-cased categories
// ("Handbags"). Never use this on values passed to categoryPath/matching —
// only on text shown to the user.
...
export function isLuggageCategory(categoryName) {
  return String(categoryName || '').toLowerCase().includes('luggage');
}
