...
const SCARCITY_MODES = [
  { value: 'off', label: 'Off — no scarcity signals' },
...
export default function ProductForm({ initial, productId, onSaved }) {
  ...
  const compressImage = (file, forOG = false) =>
    new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        const maxW = 1200; const maxH = forOG ? 630 : 1200;
  ...
  const uploadFile = async (file, forOG = false) => {
    ...
    body.append('file', compressed); body.append('bucket', 'products');
  ...
  const uploadImage = async (file, appendGallery = false) => {
    try {
      ...
      if (sizeKB > 300 && !appendGallery) toast.warning(`Image is ${sizeKB}KB — WhatsApp preview works best under 300KB.`);
      if (appendGallery) update('galleryImages', [form.galleryImages, url].filter(Boolean).join('\n'));
  ...
  const submit = async (e) => {
    ...
    const payload = {
      ...
      // Scarcity
      scarcity_mode: form.scarcity_mode,
  ...
  return (
    <form onSubmit={submit} className="bg-white border border-sethi-gray200 rounded-sm p-5 md:p-7 grid gap-5 md:grid-cols-2">
      ...
      <div className="md:col-span-2">
        <label className="block text-sm font-medium mb-1.5">Main Product Image <span className="text-sethi-gray500 font-normal text-xs">(auto-compressed to JPEG)</span></label>
      ...
      <div className="md:col-span-2">
        <label className="block text-sm font-medium mb-1.5">Gallery Images <span className="text-sethi-gray500 font-normal text-xs">(one URL per line)</span></label>
        ...
        <label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded border border-sethi-gold px-4 py-2 text-sm font-semibold text-sethi-gold hover:bg-sethi-gold hover:text-white">
          <Upload className="h-4 w-4" /> Upload gallery image
      ...
      <div className="md:col-span-2 border-t border-sethi-gray200 pt-5">
        <label className="block text-sm font-bold mb-1.5 text-sethi-black">Color Variants <span className="text-sethi-gray500 font-normal text-xs">(each color gets its own photos + in-stock toggle)</span></label>
        <div className="flex gap-2 mt-2">
          <input value={newColorName} onChange={(e) => setNewColorName(e.target.value)}
            ...
            className="input-sethi flex-1" placeholder="e.g. Red, Black, White" />
            ...
            <div key={index} className={`rounded-sm border p-4 transition-colors ${color.inStock ? 'border-sethi-gray200 bg-white' : 'border-red-200 bg-red-50'}`}>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <label className={`w-16 h-16 rounded-sm border-2 border-dashed border-sethi-gold flex items-center justify-center cursor-pointer hover:bg-sethi-gold/10 ${uploadingColor === index ? 'opacity-50 pointer-events-none' : ''}`}>
                  ...
                  <input type="file" accept="image/*" className="hidden" onChange={(e) => e.target.files?.[0] && uploadColorPhoto(index, e.target.files[0])} />
                </label>
              </div>
            </div>
          ))}
          {colorVariants.length === 0 && <p className="text-sm text-sethi-gray500 italic">No color variants added yet.</p>}
        </div>
      </div>

      {/* ── Scarcity Controls ── */}
      <div className="md:col-span-2 border-t border-sethi-gray200 pt-5">
        <label className="block text-sm font-bold mb-1 text-sethi-black">🔥 Scarcity & Urgency</label>
        <p className="text-xs text-sethi-gray500 mb-3">Show urgency signals to increase conversions. All signals are admin-controlled.</p>

        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="block text-sm font-medium mb-1.5">Scarcity Mode</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Scarcity Label <span className="text-sethi-gray500 font-normal">(shown as badge)</span></label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Display Stock <span className="text-sethi-gray500 font-normal">(shown to customer)</span></label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Stock Decay Speed <span className="text-sethi-gray500 font-normal">(random -0 to -N)</span></label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Viewers Min</label>
            <input type="number" min="1" value={form.viewing_min} onChange={(e) => update('viewing_min', e.target.value)}
              className="input-sethi" placeholder="3" disabled={!scarcityActive} />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5">Viewers Max</label>
            <input type="number" min="1" value={form.viewing_max} onChange={(e) => update('viewing_max', e.target.value)}
              className="input-sethi" placeholder="12" disabled={!scarcityActive} />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5">Price Lock (hours) <span className="text-sethi-gray500 font-normal">(0 = off)</span></label>
            <input type="number" min="0" value={form.price_lock_hours} onChange={(e) => update('price_lock_hours', e.target.value)}
              className="input-sethi" placeholder="0" disabled={!scarcityActive} />
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5">Local Scarcity</label>
            <div className="flex items-center h-12 gap-3">
              ...
              <span className="text-sm text-sethi-gray500">{form.local_scarcity ? 'Show "Popular in your area"' : 'Off'}</span>
            </div>
          </div>
        </div>
      </div>
