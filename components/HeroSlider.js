...
/** Always 6 slides — matches mockup spec */
...
function mergeSlides(apiSlides) {
  const base = DEFAULT_SLIDES.map((fallback, index) => {
...
export default function HeroSlider({ slides = [], cartCount = 0, onMenuClick, onCartClick }) {
  ...
  return (
    <section className="hero-slider-v3 w-full bg-[#faf8f4] pb-[72px] md:pb-8" data-hero="v3">
      ...
      >
        {/* Hero image panel — fixed height like mockup */}
        <div className="relative h-[min(78vh,680px)] min-h-[600px] max-h-[700px] overflow-hidden bg-[#faf8f4] sm:h-[690px] sm:min-h-[690px] sm:max-h-[690px]">
          {/* One image per slide — scenic fill in product band (no duplicate overlay) */}
          ...
          {/* Cream wash for header + readable text */}
          <div
          ...
          <header className="relative z-30 flex items-start justify-between px-7 pb-0 pt-7">
            <button type="button" onClick={onMenuClick} aria-label="Open menu" className="mt-2 text-[#111]">
            ...
            <button type="button" onClick={onCartClick} aria-label="Open cart" className="relative mt-1 text-[#111]">
        ...
        {/* White badge strip — below hero, like mockup */}
