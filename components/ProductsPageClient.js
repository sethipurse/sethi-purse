...
function effectivePrice(product) {
  return product.sale_price ?? product.salePrice ?? product.price ?? product.mrp ?? 0;
}
...
// Isolated in its own Suspense boundary so reading useSearchParams() only
// bails the (invisible) param sync out of static rendering — not the entire
// product grid, which would otherwise fall back to the loading skeleton in
// the prerendered HTML and defeat the point of server-rendering this page.
function CategoryParamSync({ onCategory }) {
  const searchParams = useSearchParams();
  useEffect(() => {
    const cat = searchParams.get('category');
    if (cat) onCategory(decodeURIComponent(cat));
  }, [searchParams, onCategory]);
  return null;
}

function ProductsContent({ categories, products }) {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-10">
      <Suspense fallback={null}>
        <CategoryParamSync onCategory={setActiveCategory} />
      </Suspense>
...
export default function ProductsPageClient({ categories, products }) {
