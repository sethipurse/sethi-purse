...
export default function MobileStickyCTA() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[9997] grid border-t border-[#ede8df] bg-white p-2 shadow-[0_-8px_24px_rgba(44,31,20,0.12)] md:hidden"
      style={{ gridTemplateColumns: isHome ? '1fr 1fr' : '1fr 1fr 1fr' }}>

      {/* Home — only on non-home pages */}
      {!isHome && (
        <Link href="/" className="flex h-12 items-center justify-center gap-1.5 rounded bg-[#f5f0e8] font-bold text-[#2c1f14]">
          <Home className="h-4 w-4 text-[#c9a84c]" /> Home
        </Link>
      )}

      <Link href="/products"
        className={`flex h-12 items-center justify-center gap-1.5 rounded bg-[#2c1f14] font-bold text-white ${!isHome ? 'mx-1' : ''}`}>
