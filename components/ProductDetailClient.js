...
// ✅ FIX: deduplicate images so uploading 1 image doesn't show it twice
function getImages(product) {
  const gallery = product.gallery_images || product.gallery || product.images || [];
  const parsed = Array.isArray(gallery)
    ? gallery
    : String(gallery || '')
        .split(',')
        .map((v) => v.trim())
...
export default function ProductDetailClient({ product, related = [], reviews = [] }) {
  ...
  // ✅ FIX: Preload image in JS before opening fullscreen
  // On Android Chrome, <img> inside position:fixed doesn't load if src is set
  // while the element is not yet in the DOM. Preloading forces the browser to
  // cache it first, so by the time MobileFullscreen mounts, the image is ready.
  const handleOpenFullscreen = () => {
    const preload = new window.Image();
    preload.onload = () => setZoomOpen(true);
    preload.onerror = () => setZoomOpen(true); // open anyway even if preload fails
      ...
      <div className="grid gap-8 lg:grid-cols-[1.02fr_0.98fr]">
        <div className="space-y-4">
          <div className="relative">
            ...
            <ZoomImage src={activeImage} alt={product.name} onClickFullscreen={handleOpenFullscreen} />
          </div>
