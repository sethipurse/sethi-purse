...
function Fullscreen({ images, startIndex, alt, onClose }) {
  ...
  const imgSrc = images[current] || images[0] || '';
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="w-11 h-11 rounded-full bg-white/20 flex items-center justify-center text-white active:bg-white/40"
      >
        <X className="w-6 h-6" />
      </button>
      ...
      {images.length > 1 && (
        <div style={{ position: 'absolute', top: 16, left: 16, zIndex: 10 }}
          className="rounded bg-black/60 px-3 py-1 text-sm text-white font-medium">
          {current + 1} / {images.length}
        </div>
      )}

      {/* Image — loading spinner until loaded */}
      <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 16px' }}>
        {!imgLoaded && (
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="w-10 h-10 border-4 border-white/20 border-t-[#c9a84c] rounded-full animate-spin" />
          </div>
        )}
        {imgSrc && (
      ...
      {images.length > 1 && (
        <>
          <button onClick={prev}
            ...
            className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center text-white active:bg-white/40">
