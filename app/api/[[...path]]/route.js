  ...
  if (segments[0] === 'reviews') {
    if (segments.length === 1) {
      ...
      if (method === 'POST') {
        ...
        const googleLink = r.google_review_link ?? r.googleReviewLink;
        const review = { id: uuidv4(), customer_name: String(r.customerName).trim(), customer_photo: String(r.customerPhoto || '').trim(), rating, review_text: String(r.reviewText).trim(), is_featured: !!r.isFeatured, category: r.category ? String(r.category).trim() : null, google_review_link: googleLink ? String(googleLink).trim() : null, created_at: nowIST() };
